...
def _run_attest(func_name: str, code: str, axioms_path: str, safe: bool | None) -> None:
    ...
    import hashlib
    from .core.attestation import get_engine
...
@cli.command("pcm-generate")
@click.argument("sil_file", type=click.Path(exists=True))
@click.option("--axioms", default="config/axioms.yaml", show_default=True)
@click.option("--out", default=None, help="Write proof JSON to this file.")
@click.option("--json-output", is_flag=True, default=False)
def pcm_generate(sil_file: str, axioms: str, out: str | None, json_output: bool) -> None:
    """
    Generate a PPL proof for a SIL file.

    The agent must submit this proof alongside every code modification
    in PCM mode.  The proof is generated by a rule-based system that
    walks the SIL AST and emits proof steps for each safety axiom.
    """
    from .axioms.axiom_parser import AxiomParser
    from .pcm.proof_generator import ProofGenerator
    ...
    result = generator.generate(program, axiom_list)

    proof_json = json.dumps(result.proof, indent=2)

    if out:
        with open(out, "w") as f:
            f.write(proof_json)
        click.echo(f"Proof written to {out}")
    elif json_output:
        click.echo(proof_json)
    else:
        click.echo("\nPCM Proof Generation")
        click.echo(f"  Function      : {result.function}")
        click.echo(f"  Safe          : {result.is_safe}")
        click.echo(f"  Axioms covered: {result.axioms_covered}")
        click.echo(f"  Steps         : {len(result.proof.get('steps', []))}")
        click.echo(f"  Conclusion    : {result.proof.get('conclusion')}")
        if result.generation_error:
            click.echo(f"  Error         : {result.generation_error}", err=True)
...
@cli.command("pcm-verify")
@click.argument("proof_file", type=click.Path(exists=True))
@click.option("--axioms", default="config/axioms.yaml", show_default=True)
@click.option("--json-output", is_flag=True, default=False)
def pcm_verify(proof_file: str, axioms: str, json_output: bool) -> None:
    """
    Verify a PPL proof without running the full PAAC pipeline.

    The proof checker validates each proof step using pure symbolic
    evaluation -- no Z3 is called.  Target: < 10ms per proof.
    """
    from .axioms.axiom_parser import AxiomParser
    ...
    axiom_dicts = [{"id": a.id, "condition": a.condition} for a in axiom_list]
    ...
    if json_output:
            ...
            json.dumps(
                {
                    ...
                    "covered_axioms": result.covered_axioms,
        ...
        click.echo(f"\nPCM Proof Verification: {status}")
        click.echo(f"  Reason        : {result.reason}")
        click.echo(f"  Covered axioms: {result.covered_axioms}")
        click.echo(f"  Elapsed       : {result.elapsed_ms:.2f}ms")
...
@cli.command("pcm-submit")
@click.argument("sil_file", type=click.Path(exists=True))
@click.argument("proof_file", type=click.Path(exists=True))
@click.option("--axioms", default="config/axioms.yaml", show_default=True)
@click.option("--agent-id", default="cli-agent", show_default=True)
@click.option("--mod-id", default=None, help="Modification ID (auto-generated if omitted).")
@click.option("--cert-out", default=None, help="Write certificate JSON to this file.")
@click.option("--log", default="pcm_audit.jsonl", show_default=True)
@click.option("--json-output", is_flag=True, default=False)
def pcm_submit(
    sil_file: str,
    proof_file: str,
    ...
    """
    Submit a SIL modification with its proof to PAAC.

    PAAC verifies the proof (not the code) and issues a certificate
    if the proof is accepted.  The certificate is appended to the
    ...
    axiom_dicts = [{"id": a.id, "condition": a.condition} for a in axiom_list]
    checker = ProofChecker(axiom_dicts)
    check_result = checker.check(proof)

    if not check_result.accepted:
        if json_output:
            click.echo(
                json.dumps(
                    {"accepted": False, "reason": check_result.reason, "failed_step": check_result.failed_step},
                    indent=2,
                )
            )
        else:
            click.echo(f"Proof REJECTED: {check_result.reason}", err=True)
        sys.exit(1)

    effective_mod_id = mod_id or f"{sil_file}:{int(time.time())}"
    cert = generate_certificate(
        modification_id=effective_mod_id,
        code=code,
        proof=proof,
        agent_id=agent_id,
    ...
    store = CertificateStore(log_path=log)
    store.save(cert)

    if cert_out:
        with open(cert_out, "w") as f:
            f.write(cert.to_json())
        click.echo(f"Certificate written to {cert_out}")
            ...
            json.dumps(
                {
                    ...
                    "axioms_covered": cert.axioms_covered,
                    "timestamp": cert.timestamp,
                    "check_elapsed_ms": round(check_result.elapsed_ms, 3),
                },
                indent=2,
            )
        )
    else:
        click.echo("\nPCM Submission: ACCEPTED")
        click.echo(f"  Modification ID: {cert.modification_id}")
        click.echo(f"  Code hash      : {cert.code_hash[:16]}...")
        click.echo(f"  Proof hash     : {cert.proof_hash[:16]}...")
        click.echo(f"  Axioms covered : {cert.axioms_covered}")
        click.echo(f"  Check time     : {check_result.elapsed_ms:.2f}ms")
        click.echo(f"  Logged to      : {log}")
...
@cli.command("pcm-audit")
@click.option("--log", default="pcm_audit.jsonl", show_default=True)
@click.option("--mod-id", default=None, help="Query a specific modification ID.")
@click.option("--json-output", is_flag=True, default=False)
def pcm_audit(log: str, mod_id: str | None, json_output: bool) -> None:
    """
    Query the PCM certificate audit log.

    Lists all certificates or queries a specific modification ID.
    Verifies the HMAC signature of each certificate.
    """
    from .pcm.certificate import CertificateStore, verify_certificate

    store = CertificateStore(log_path=log)

    if mod_id:
        cert = store.query(mod_id)
        if cert is None:
            click.echo(f"No certificate found for modification_id='{mod_id}'", err=True)
            sys.exit(1)
        check = verify_certificate(cert)
        if json_output:
            d = cert.to_dict()
            d["verification"] = {"valid": check.valid, "checks_passed": check.checks_passed, "checks_failed": check.checks_failed}
            click.echo(json.dumps(d, indent=2))
        else:
            status = "VALID" if check.valid else "INVALID"
            click.echo(f"Certificate [{status}]: {cert.modification_id}")
            click.echo(f"  Agent    : {cert.agent_id}")
            click.echo(f"  Timestamp: {cert.timestamp}")
            click.echo(f"  Axioms   : {cert.axioms_covered}")
        sys.exit(0 if check.valid else 1)
    ...
    if json_output:
        output = []
        for c in certs:
            check = verify_certificate(c)
            d = c.to_dict()
            d["valid"] = check.valid
            output.append(d)
        click.echo(json.dumps({"count": len(output), "certificates": output}, indent=2))
    else:
        click.echo(f"\nPCM Audit Log: {log}")
        click.echo(f"  Total certificates: {len(certs)}")
        for c in certs:
            check = verify_certificate(c)
            status = "OK" if check.valid else "TAMPERED"
            click.echo(f"  [{status}] {c.modification_id} @ {c.timestamp} agent={c.agent_id}")
