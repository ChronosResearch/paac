...
class SILParser:
    ...
    def consume(self, expected_type: str = None, expected_value: str = None) -> Token:
        ...
        if expected_type and tok.type != expected_type:
            raise SILError(f"Expected token type {expected_type}, got {tok.type} at {tok.line}:{tok.column}")
        if expected_value and tok.value != expected_value:
            raise SILError(f"Expected '{expected_value}', got '{tok.value}' at {tok.line}:{tok.column}")
        self.pos += 1
        return tok
    
    def match(self, expected_type: str = None, expected_value: str = None) -> bool:
        tok = self.peek()
    ...
    def parse_binary_expr(self, precedence: int) -> ASTNode:
        ...
        while True:
            tok = self.peek()
            if not tok or tok.type not in ('OPERATOR', 'KEYWORD'): break
            if tok.value not in ('+', '-', '*', '/', '==', '!=', '<', '<=', '>', '>=', 'and', 'or'): break
        ...
        if isinstance(stmt, AssignmentStmtNode):
            ...
            if stmt.target in self.current_env:
                if self.current_env[stmt.target] != val_type:
                    raise SILError(f"Type mismatch in assignment to {stmt.target}: {val_type} != {self.current_env[stmt.target]}")
            else:
            ...
            for s in stmt.then_branch: self._check_stmt(s, return_type)
            for s in stmt.else_branch: self._check_stmt(s, return_type)
        elif isinstance(stmt, WhileStmtNode):
            cond_type = self._check_expr(stmt.condition)
            if cond_type != 'bool':
                raise SILError(f"While condition must be bool, got {cond_type}")
            for s in stmt.body: self._check_stmt(s, return_type)
        elif isinstance(stmt, ReturnStmtNode):
            val_type = self._check_expr(stmt.value)
            if val_type != return_type:
                raise SILError(f"Return type mismatch: expected {return_type}, got {val_type}")
    ...
    def _check_expr(self, expr: ASTNode) -> str:
        if isinstance(expr, LiteralNode):
            ...
            if expr.operator in ('+', '-', '*', '/', '<', '<=', '>', '>='):
                ...
                if l_type != r_type:
                    raise SILError(f"Operator {expr.operator} requires operands of same type")
                return 'bool'
...
class SILToIRCompiler:
    ...
    def _compile_stmt(self, stmt: ASTNode, current_block: BasicBlock) -> BasicBlock:
        if isinstance(stmt, (AssignmentStmtNode, ReturnStmtNode, AssertStmtNode)):
            ...
            t_curr = then_block
            for s in stmt.then_branch: t_curr = self._compile_stmt(s, t_curr)
            t_curr.successors.append(merge_block)
            
            e_curr = else_block
            for s in stmt.else_branch: e_curr = self._compile_stmt(s, e_curr)
            ...
            b_curr = body
            for s in stmt.body: b_curr = self._compile_stmt(s, b_curr)
