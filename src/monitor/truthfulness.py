...
class TruthfulnessEnforcer:
    ...
    def enforce_citations(self, citations: List[str]):
        ...
        if not citations:
            raise HallucinationError("Zero citations provided. Must cite sources for safety modifications.")
    ...
    def validate_modification(self, llm_response: str) -> bool:
        ...
        try:
            ...
            if self.detect_hallucination_heuristics(data["reasoning"]):
                raise HallucinationError("Uncertain reasoning detected - potential hallucination.")
                
            self.logger.info(f"ACCEPTED_MODIFICATION: {data['modification']} CITATIONS: {data['citations']}")
            return True
        except HallucinationError as e:
            self.logger.warning(f"REJECTED_HALLUCINATION: {str(e)}")
            return False
